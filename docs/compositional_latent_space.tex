...
Re-dependent flow structures --- a candidate explanation for the
residual $\sim 0.3$ probe leakage.

\paragraph{Planned increments, in order.}
%
\begin{itemize}
  \item \textbf{Encoder-side invariance test}: feed the encoder the SDF
        alongside the flow (or add an encoder invariance term) to test
        whether the remaining Re-to-geometry cross-talk and the
        $\sim 0.3$ probe leakage share the flow-appearance mechanism
        identified by the concept-vector diagnostic.
  \item \textbf{Residual capacity}: shrink the residual block (e.g.\
        ...
        content remaining in $\boldsymbol{z}_{\xi}$ ($R^2 \approx 0.2$).
