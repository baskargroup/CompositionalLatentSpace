...
whereas our M2 is flat ($\approx$ M1). This is the expected cost of our
$256^2$ native resolution, which smooths exactly the thin near-surface
layers that M2 measures --- and it points directly at two queued
improvements: training at full resolution, and the boundary-layer
weighted reconstruction loss (L3), which concentrates training signal in
the M2 region.
...
with the boundary layer (M2) identified as the resolution-limited
weakness.

\paragraph{Planned increments, in order.}
%
\begin{itemize}
  \item \textbf{Boundary-layer weighted reconstruction (L3)}, directly
        motivated by the flat M2 score: an additional reconstruction
        term restricted to the band $0 \le \mathrm{SDF} \le 0.2$ and
        normalized by the band's own pixel count, so the thin near-wall
        region --- a few percent of the fluid pixels, diluted away under
        the plain loss --- carries real training weight. Implemented as
        Run 8 (the Run-7 recipe plus this term); training queued.
