...
Five training runs to date, each changing one ingredient at a time so
...
recombination respectively. Run 5 is the current best configuration. The
following subsections document each run in chronological order, each
ending with the reasoning that led to the next run.

\begin{table}[htbp]
  \centering
  \caption{Summary of all runs. ``Leakage'' is the linear-probe $R^2$ of
  $\log \mathrm{Re}$ read from $\boldsymbol{z}_{g}$, where it does not
  belong (target $\lesssim 0.3$); ``swap ratio'' is the same-Re swap
  error relative to the reconstruction error (target $\approx 1$; defined
  in Section~\ref{sec:swap}).}
  \label{tab:overview}
  \begin{tabular}{llccl}
    \toprule
...
The leakage has dropped by more than half, but $R^2 = 0.356$ still means
that roughly a third of the Reynolds information sits in a block where it
does not belong. It is worth noticing that neither L10 nor L12 targets
$\boldsymbol{z}_{\xi}$ directly; the improvements so far are
\emph{side effects}. Once L10 forced $\boldsymbol{z}_{g}$ to be Re-free,
the decoder had to rely more on $\boldsymbol{z}_{\mu}$ for the operating
point, and the now-less-useful copies elsewhere partly decayed.

Why this matters beyond tidiness: the leftover Re in
$\boldsymbol{z}_{\xi}$ is our prime suspect for the cross-Re transfer
...
Run 6 is the first run that attacks this directly. Its cross-Re training
...
pressure should squeeze it out. When Run 6's probe table arrives, the
$\log \mathrm{Re}$-from-$\boldsymbol{z}_{\xi}$ cell is one of the two
numbers to watch, alongside the cross-Re ratio itself. If Run 6 does not
clean it, two follow-ups are already queued: shrink the residual block
from 16 to 4 dimensions (less room to hide Re), and the HSIC penalty
(which catches nonlinear dependence that the Pearson penalty misses).
...
no cost to reconstruction}. Moreover the two properties are genuinely
distinct: each requires its own loss, and each is invisible to the other
property's diagnostic.

\paragraph{Planned increments, in order.}
%
\begin{itemize}
  \item \textbf{Cross-Re swap measurement} on the Run-1/4/5 checkpoints
        (done --- all models fail; see Table~\ref{tab:xswap}), leading to
        \textbf{Run 6}: cross-Re swapping promoted from diagnostic to
        training loss (in progress).
  \item \textbf{Multi-seed protocol}: rerun the Run-4 and Run-5
        configurations with 2--3 different random seeds to put error bars
        on the probe tables and settle the small differences (e.g.\
        leakage $0.215$ vs.\ $0.343$) currently attributed to
        run-to-run variation.
  \item \textbf{Concept-vector arithmetic} on $\boldsymbol{z}_{\mu}$ and
        $\boldsymbol{z}_{g}$: walk the latent along the direction
        associated with one parameter and check that only that parameter
        responds (the AeroJEPA-style sensitivity matrix; a Week-15
        success criterion of the project plan).
  \item \textbf{Residual capacity}: shrink the residual block (e.g.\
        $16 \to 4$ dimensions) --- a steady, deterministic dataset needs
        little residual capacity, and less room may squeeze out the Re
        content remaining in $\boldsymbol{z}_{\xi}$ ($R^2 \approx 0.36$).
  \item \textbf{Loss library extensions}: HSIC (L7), which detects
        nonlinear dependence between blocks, not just linear correlation;
        boundary-layer weighted reconstruction (L3), which weights errors
        near the object surface more heavily.
