...
\begin{figure}[htbp]
  ...
  magnitude smaller than the fields (fourth column). The regime code
  alone moved this geometry across two decades of Reynolds number.}
  \label{fig:transfer}
\end{figure}
...
%
\begin{itemize}
  \item \textbf{Figures}: field visualizations for the paper ---
        reconstruction examples (truth vs.\ prediction vs.\ error) and,
        above all, the cross-Re transfer figure: a decoded
        geometry-at-new-Re next to the CFD ground truth.
  \item \textbf{FlowBench baseline comparison}: one table comparing our
        reconstruction error against the published FlowBench neural
        operator baselines (success criterion 1 of the project plan).
  \item \textbf{Multi-seed for Run 7} (two more seeds) to put error bars
        on the new architecture's numbers.
  ...
  \item \textbf{Residual capacity}: shrink the residual block (e.g.\
        ...
        ($R^2 \approx 0.69$) but functionally inert; a smaller block
