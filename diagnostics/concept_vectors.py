...
def main(config_path, checkpoint_path, n_samples=64, step=1.0, seed=0):
    ...
    # 3. One ridge probe per parameter on the full latent; the weight
    #    direction is the parameter's concept vector
    probes = {k: Ridge(alpha=1.0).fit(Z, y) for k, y in targets.items()}
    directions = {k: p.coef_ / np.linalg.norm(p.coef_) for k, p in probes.items()}
    ...
    base_pred = {k: probes[k].predict(z_cycle) for k in names}
    ...
    for a, k in enumerate(names):
        ...
        alpha = step / np.linalg.norm(probes[k].coef_)
        ...
        for b, j in enumerate(names):
            S[a, b] = (probes[j].predict(z_walk_cycle) - base_pred[j]).mean() / step
    ...
    out_path = os.path.join(out_dir, 'concept_matrix_test.csv')
